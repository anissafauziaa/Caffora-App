...
require_login(['admin']); // hanya admin boleh akses
...
<head>
  <meta charset="utf-8">
  <title>Kelola Pesanan — Admin Desk</title>
...
<script>
(() => {
  // ===== Nav aktif saat diklik (konsisten dengan dashboard) =====
  ...
  const API = '<?= rtrim(BASE_URL, "/") ?>/backend/api/orders.php';
    ...
    try{
      ...
      $rows.querySelectorAll('[data-act]').forEach(btn=>{
        btn.addEventListener('click', async (e)=>{
          ...
          if (!res2.ok || !js2.ok) alert(js2.error || ('HTTP '+res2.status));
          await load();
        });
      });
